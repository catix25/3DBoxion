...
export function Icon3D({ iconData }: Icon3DProps) {
  ...
  const iconTexture = useMemo(() => {
    ...
    if (ctx) {
      ...
      if (iconData.style === "gradient") {
        ...
        gradient.addColorStop(0, iconData.background)
  ...
  const scale = 0.003
  const iconDimensions: [number, number, number] = [
    iconData.dimensions.width * scale,
    iconData.dimensions.height * scale,
    iconData.dimensions.depth * scale,
  ]

  // Crear geometría según la forma
  const geometry = useMemo(() => {
    if (iconData.shape === "circle") {
      return <cylinderGeometry args={[iconDimensions[0] / 2, iconDimensions[0] / 2, iconDimensions[2], 32]} />
    } else {
      return <boxGeometry args={iconDimensions} />
    }
  }, [iconData.shape])
  ...
  return (
    <group>
      ...
      {iconData.glow && (
        <mesh>
          <boxGeometry args={[iconDimensions[0] + 0.1, iconDimensions[1] + 0.1, iconDimensions[2] + 0.1]} />
